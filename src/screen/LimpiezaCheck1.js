...
export default function Solicitud(props) {
	...
	const [tituloApi, setTituloApi] = useState(null);
	const [nombreApi, setNombreApi] = useState(null);
	const [imagenApi, setImagenApi] = useState(null);

	useEffect(() => {
		const fetchData = async () => {
		  try {
			const apiUrl = `postulaciones/postulacion/${idPostulacion}`;
			const response = await axios.get(URL_API + apiUrl, config);
			setPostulacionData(response.data);
			setIdUsuario(response.data[0][3]);
			setTituloApi(response.data[0][1]);
			setNombreApi(response.data[0][2]);
			setImagenApi(response.data[0][4]);
			console.log('Primer valor:', response.data[0][3]);
	...
	<View style={{ flex: 2 }}>
	  ...
	  <View style={styles.containerSvg}>
		<View style={{marginTop:20}}>
				...
				<View>
				<Image
			 		source={{ uri: imagenApi }}
			 		style={{ width: 200, height: 200, borderRadius:130, borderWidth:7, borderColor:'#05668D'}}
		 		/>
				</View>
				
		 		<View style={{marginTop:20}}>
				 <View style={styles.ContainerStatus}>
				<Text style={styles.name}><Icon name="briefcase" size={24} style={styles.icon} /> {tituloApi}</Text>
				</View>
				<View style={styles.ContainerStatus}>
				<Text style={styles.name}><Icon name="user" size={24} style={styles.icon} />{nombreApi}</Text>
				</View>
			</View>
